...
function validateTierInput(body) {
  const { name, min_points, discount_percent } = body;
  ...
  if (
    ...
    minPoints < 0 ||
    !Number.isInteger(minPoints)
  ) {
    return 'min_points is required and must be an integer >= 0';
  }
...
/** Authenticated patients + staff — ladder for membership progress UI. */
const getActiveTiers = async (req, res) => {
  try {
    const tiers = await MembershipTier.getActive();
    res.json(tiers);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const createTier = async (req, res) => {
  try {
    const { name, min_points, discount_percent, is_active } = req.body;
    const validationError = validateTierInput(req.body);
    if (validationError) {
      return res.status(400).json({ message: validationError });
    }
    const tierData = { name, min_points, discount_percent, is_active };
