...
class Schedule {
  static async getByOrderId(orderId) {
    ...
    const result = await pool.request()
      ...
      .query('SELECT * FROM order_schedules WHERE order_id = @order_id');
  ...
  static async upsert(data) {
    ...
    const result = await pool.request()
      ...
      .query(`
        ...
        ELSE
        BEGIN
            INSERT INTO order_schedules (id, order_id, collecting_person, collection_time, running_time, report_out_time, accepted_by_user)
            OUTPUT INSERTED.*
            VALUES (NEWID(), @order_id, @collecting_person, @collection_time, @running_time, @report_out_time, @accepted_by_user)
        END
      `);
    return result.recordset[0];
  }
