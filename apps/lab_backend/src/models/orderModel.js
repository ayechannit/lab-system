...
class Order {
  ...
  static async syncPendingOrder(id, data, updatedBy, userType) {
    const pool = await poolPromise;
    const transaction = new sql.Transaction(pool);
    ...
    try {
      ...
      // 1. Fetch current order to check status and creator
      const orderReq = new sql.Request(transaction);
      const currentOrderResult = await orderReq
        .input('order_id', sql.UniqueIdentifier, id)
        .query('SELECT status, user_id, created_user FROM lab_orders WHERE id = @order_id AND is_deleted = 0');
      
      const order = currentOrderResult.recordset[0];
      if (!order) {
        throw new Error('Order not found');
      }
      
      // 2. Validate status and creator
      if (order.status !== 'pending') {
        const err = new Error('Order can only be updated when its status is pending');
        err.statusCode = 403;
        throw err;
      }
      
      const isStaff = userType === 'staff';
      const isCreator = (order.created_user && String(order.created_user).toLowerCase() === String(updatedBy).toLowerCase()) ||
                        (order.user_id && String(order.user_id).toLowerCase() === String(updatedBy).toLowerCase());
      
      if (!isStaff && !isCreator) {
        const err = new Error('Access denied. Only the order creator or staff can update this order.');
        err.statusCode = 403;
        throw err;
      }
      
      // 3. Update the main order fields and price totals
      const updateOrderReq = new sql.Request(transaction);
      const originalPrice = data.original_price_mmk || 0;
      const discountPercent = data.discount_percent || 0;
      const finalPrice = data.final_price_mmk || 0;
      ...
      await deletePaymentsReq
        .input('order_id', sql.UniqueIdentifier, id)
        .query("DELETE FROM payments WHERE order_id = @order_id AND status = 'pending'");
      ...
      if (data.payments && data.payments.length > 0) {
        for (const payment of data.payments) {
          ...
          await payReq
            .input('order_id', sql.UniqueIdentifier, id)
            .input('amount_mmk', sql.Decimal(18, 2), payment.amount_mmk)
            .input('status', sql.VarChar, payment.status || 'pending')
            .input('method', sql.VarChar, payment.method)
            .input('reference_no', sql.VarChar, payment.reference_no ?? null)
            .input('created_user', sql.UniqueIdentifier, updatedBy)
            .query(`
      ...
      // Fetch and return the fully populated updated order
